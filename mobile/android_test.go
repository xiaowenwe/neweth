...
// androidTestClass是一个Java类，可以对Android绑定进行一些轻量级测试。 目标不是测试每个单独的功能，而只是为了捕获破坏API和/或实现更改。
...
// TestAndroid运行上面指定的Android java测试类。
//这需要PATH中的gradle命令和Android SDK，其路径可通过ANDROID_HOME环境变量获得。 要成功运行测试，还必须启用Android设备并启用调试。
//这个方法改编自golang.org/x/mobile/bind/java/seq_test.go/runTest
